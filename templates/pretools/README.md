...
Pretools use the same `BaseTool` base class as regular tools. The difference is configuration: they are listed in the agent's `pretools` attribute and use a separate `pregeneration_config` optimized for fast execution.
...
| `DailyTipTool` | Local data rotation | Rotates through CogSol Framework tips based on the current date |
