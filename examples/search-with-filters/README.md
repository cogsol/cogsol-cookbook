...
For a simpler version without filters, see the `search-no-filters` example.
...
- See the `search-no-filters` example for a simpler version without metadata filtering.
